...
const getLatestEvents = async (contract, limit = 12) => {
  ...
  while (from > 0 && events.length < limit) {
    console.debug(
      `searching range - ${JSON.stringify({ from, to, pageLimit: PAGE_LIMIT })} - eventsFound=${events.length}`
    )
...
export const usePublicMintTokenMutation = () => {
  ...
  const queryClient = useQueryClient()
  return useMutation(
    async ({ countOfChickens, totalPrice }) => {
...
export const useSetTokenSalePriceMutation = (contract, enabled = true) => {
  const queryClient = useQueryClient()
  return useMutation(
    async ({ tokenId, newPrice, isForSale }) => {
      return new Promise((resolve, reject) => {
        const ethPrice = utils.parseUnits(newPrice.toString(), 'ether')
        contract
          .setPriceForSale(tokenId, ethPrice, isForSale)
          .then((tx) => {
...
export const useBuyTokenMutation = (contract, enabled = true) => {
  const queryClient = useQueryClient()
  return useMutation(
    async ({ tokenId, salePrice }) => {
      return new Promise((resolve, reject) => {
        contract
          .buyToken(tokenId, { value: utils.parseUnits(salePrice.toString(), 'ether') })
          .then((tx) => {
...
export const useStoreWorkingTxQuery = (currentTx) => {
  return useQuery(KEYS.TRANSACTION(), () => {
    return new Promise((resolve, reject) => {
      resolve({})
    })
  })
...
export const useAirdropMutation = (contract, enabled = true) => {
  ...
  // status : boolean
  return useMutation(
    async ({ numberOfToken, address }) => {
      return new Promise((resolve, reject) => {
        contract
          .airdropTokens(numberOfToken, address)
          .then((tx) => {
...
export const useSetKGMutation = (contract, enabled = true) => {
  ...
  return useMutation(
    async ({ tokenId, kg }) => {
      return new Promise((resolve, reject) => {
        contract
          .setKg(parseInt(tokenId), parseInt(kg))
          .then((tx) => {
...
export const useGetChickenDetailMutation = (contract, enabled = true) => {
  ...
  return useMutation(
    async ({ tokenId }) => {
      return new Promise((resolve, reject) => {
        contract
          .allChickenRun(parseInt(tokenId))
          .then((tokenDetail) => {
...
export const useAPIMarketStat = (showForSale = false) => {
  return useQuery(
    ...
    async () =>
      axios
        .get(`${siteConfig.apiUrl}/api/market/list?forSale=${showForSale ? 'true' : 'false'}`)
        .then((res) => res.data),
    {
      cacheTime: 15 * 1000,
      staleTime: 15 * 1000,
